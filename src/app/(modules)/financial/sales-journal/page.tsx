...
import { Receipt, Download, DollarSign, BookOpen, TrendingUp, ArrowRightLeft } from 'lucide-react';
...
export default function SalesJournalPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/financial/sales-journal?${params}`);
      const json = await res.json();
      setData(json);
    } catch (e) { console.error('Failed to fetch sales journal', e); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchCustomers(); }, []);
  useEffect(() => { fetchData(); }, [customerId, dateFrom, dateTo]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Double-entry journal of all customer invoices</p>
        </div>
                ...
                <Table>
                  ...
                  <tfoot>
                    <TableRow className="font-bold bg-slate-50 border-t-2 border-slate-300">
                      <TableCell colSpan={4} className="text-sm text-slate-700">Totals ({data?.totals.entryCount} invoices)</TableCell>
