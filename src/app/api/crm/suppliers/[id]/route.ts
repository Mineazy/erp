...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const item = await prisma.erpSupplier.findUnique({ where: { id: params.id } });
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.erpSupplier.findUnique({ where: { id: params.id } });
  if (!existing) return notFound('Supplier not found');

  const body = await getBody(request);
  const { name, contactPerson, email, phone, address, city, country, taxId, paymentTerms, notes, isActive } = body;
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.erpSupplier.delete({ where: { id: params.id } });
